...
async def create_agents():
    ...
    stock_finder_agent = create_react_agent(model, tools, prompt=""" You are an equity research analyst for the Indian stock market (NSE). Select exactly 2 promising NSE-listed stocks for short-term trading based on recent performance, market sentiment, news, and momentum. Rules:
    ...
    Do not include reasoning. Do not include markdown. Do not include explanations.""", response_format=StockFinderOutput, name = "stock_finder_agent")
    ...
    Do not include reasoning. Do not include markdown. Do not include explanations.""", name = "market_data_agent")

    news_analyst_agent = create_react_agent(model, tools, prompt="""You are a financial news analyst. For each provided NSE stock:
    - Find important news from the last 3-5 days.
    ...
    Output one section per stock.""", name = "news_analyst_agent")
    ...
    Return only the final recommendation.""", name = "price_recommender_agent")
